...
const drawFn = async () => {
    // 查询今日是否有免费抽奖机会
    // const today1 = await fetch(`https://api.juejin.cn/growth_api/v1/lottery_config/get`, {
    //     headers,
    //     method: 'GET',
    //     credentials: 'include'
    // }).then((res) => res.text());
    // console.log(today1, typeof today1,"lottery_config/get" );
    // let today = {};
    // try{
    //     today = JSON.parse(today1)
    // }catch(e){
    ...
    // }
    console.log("先无视掉这个异常，看看后面能否执行1");
    ...
    console.log(draw, 'draw', draw.length);
    return;

    if (draw.err_no !== 0) return Promise.reject('已经签到！免费抽奖异常！');
    console.log(JSON.stringify(draw, null, 2), "lottery/draw");
...
(async () => {
    ...
    console.log(today_status, 'get_today_status');
    ...
    .then((res) => {
        console.log(res, 'get_cur_point');
        score = res.data;
        // return drawFn();
    })
    .then((msg) => {
        console.log(msg);
        notify.sendNotify(`掘金签到通知`,`签到结果1：${msg}\n`,`当前积分：${score}\n`)
    })
    .then(() => {
        // console.log('邮件发送成功！');
    })
    .catch((err) => {
        console.warn(err, 'error catch');
        notify.sendNotify(`掘金签到通知`,`签到结果2：${err}\n`,`当前积分：${score}\n`)
