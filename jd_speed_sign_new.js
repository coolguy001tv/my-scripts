/*
cron "10 8,13,19 * * *" jd_speed_sign_new.js, tag:京东特价版任务
*/
//详细说明参考 https://github.com/ccwav/QLScript2.

// add:去除掉不需要的cookie
// 参考https://github.com/coolguy001tv/my-scripts
...
// 不需要的pin列表请用&分开
const removePins = (process.env.JD_SPEED_REMOVE_PINS||'').split('&');
if(removePins?.length){
  cookiesArr = cookiesArr.filter(one=>{
    const has = removePins.find(pin=>{
      return one.includes(pin);
    })
    if(has){
      return false;
    }
